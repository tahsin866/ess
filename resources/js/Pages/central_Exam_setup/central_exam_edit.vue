    ...
    <AuthenticatedLayout>
     ...
     <tbody>
                            <tr class="hover:bg-emerald-50 transition">
                                <td class="border border-emerald-300 p-3 text-center font-medium">1</td>
                                <td class="border border-emerald-300 p-3 font-medium text-emerald-900">
                                    {{ props.examSetup.exam_name }}
                                </td>

                                <!-- Registration Fee Section -->
                                <td class="border border-emerald-300 p-3">
                                    <div class="flex items-center gap-1">
                                        <input type="date" v-model="form.reg_date_from"
                                               class="w-28 border border-emerald-200 p-2 rounded-sm">
                                        <span class="whitespace-nowrap">থেকে</span>
                                        <input type="date" v-model="form.reg_date_to"
                                               class="w-28 border border-emerald-200 p-2 rounded-sm">
                                    </div>
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.reg_regular_fee"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.reg_irregular_jemni"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.reg_irregular_manonnoyon"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.reg_irregular_others"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>
                                ...
                                <td class="border border-emerald-300 p-3">
                                    <div class="flex items-center gap-1">
                                        <input type="date" v-model="form.late_date_from"
                                               class="w-28 border border-emerald-200 p-2 rounded-sm">
                                        <span class="whitespace-nowrap">থেকে</span>
                                        <input type="date" v-model="form.late_date_to"
                                               class="w-28 border border-emerald-200 p-2 rounded-sm">
                                    </div>
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.late_regular_fee"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.late_irregular_jemni"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.late_irregular_manonnoyon"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>

                                <td class="border border-emerald-300 p-3">
                                    <input type="number" v-model="form.late_irregular_others"
                                           class="w-full border border-emerald-300 p-2 rounded-sm">
                                </td>
                            </tr>
                        </tbody>
...
const form = useForm({
    reg_date_from: props.examFee.reg_date_from,
    reg_date_to: props.examFee.reg_date_to,
    reg_regular_fee: props.examFee.reg_regular_fee,
    reg_irregular_jemni: props.examFee.reg_irregular_jemni,
    reg_irregular_manonnoyon: props.examFee.reg_irregular_manonnoyon,
    reg_irregular_others: props.examFee.reg_irregular_others,
    late_date_from: props.examFee.late_date_from,
    late_date_to: props.examFee.late_date_to,
    late_regular_fee: props.examFee.late_regular_fee,
    late_irregular_jemni: props.examFee.late_irregular_jemni,
    late_irregular_manonnoyon: props.examFee.late_irregular_manonnoyon,
    late_irregular_others: props.examFee.late_irregular_others
})

const formattedTitle = computed(() => {
    return `${props.examSetup.exam_name} (${props.examSetup.bangla_year}/${props.examSetup.arabic_year}/${props.examSetup.english_year})`
})

// Update your table inputs to use form directly
const submit = () => {
    form.put(route('central_Exam_setup.update', props.examFee.id))
}
